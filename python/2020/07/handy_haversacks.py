...
def handy_haversacks_part_2(rules):
  rules = rules.split('\n')
  color_contents = {}
  ...
  for rule in rules:
    ...
    for bags in contents.split(', '):
      count, nested_color = re.findall(r'^(\d) (.*) bag', bags)[0]
      nested_color_counts[nested_color] = int(count)
    color_contents[color] = nested_color_counts
  ...
  return get_nested_count('shiny gold')